...
function BangaloreQuestionsPanel() {
  return (
    <div className="z-card border-l-4 border-cyan-500 bg-cyan-50">
      <div className="z-eyebrow">📋 Your Bangalore questions — read verbatim during Step 2 / Talking Point 2</div>
      <p className="mt-2 text-xs leading-relaxed text-zillow-slate">
        These are the actual questions you wrote down after reading the Bangalore BOM. Each
        is framed as a clarification, not a critique. Read them in order, pause after each,
        let Stacey direct attention.
      </p>

      <div className="mt-4 space-y-4">
        {BANGALORE_QUESTIONS.map((q) => (
          <div key={q.number} className="rounded-lg border border-cyan-200 bg-white p-4">
            <div className="flex items-baseline gap-3">
              <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-cyan-600 text-sm font-bold text-white">
                {q.number}
              </span>
              <div className="flex-1">
                ...
                <div className="mt-3 rounded-md bg-zillow-gray-light px-3 py-2 text-xs leading-relaxed text-zillow-slate">
                  <span className="font-semibold text-zillow-ink">Why ask: </span>
                  {q.whyAsk}
                </div>
                <div className="mt-2 grid gap-2 md:grid-cols-2">
                  <div className="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs leading-relaxed">
                    <div className="font-semibold text-emerald-700">
                      ↳ If she has an answer
                    </div>
                    <div className="mt-1 text-zillow-ink">{q.ifSheKnows}</div>
                  </div>
                  <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-relaxed">
                    <div className="font-semibold text-amber-800">
                      ↳ If she doesn&apos;t know
                    </div>
                    <div className="mt-1 text-zillow-ink">{q.ifSheDoesnt}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-baseline gap-3">
            ...
            <div className="flex-1">
              <div className="text-xs font-semibold uppercase tracking-wider text-amber-800">
                Spec suggestion (the &ldquo;1 small thing&rdquo;) — {BANGALORE_SPEC_SUGGESTION.topic}
              </div>
              <p className="mt-2 text-sm font-medium leading-relaxed text-zillow-ink">
                ...
                <br />
                {BANGALORE_SPEC_SUGGESTION.verbatim}
              </p>
              <div className="mt-3 rounded-md bg-white px-3 py-2 text-xs leading-relaxed text-zillow-slate">
                ...
                {BANGALORE_SPEC_SUGGESTION.whyAsk}
              </div>
              <div className="mt-2 grid gap-2 md:grid-cols-2">
                <div className="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs leading-relaxed">
                  <div className="font-semibold text-emerald-700">↳ If she likes it</div>
                  <div className="mt-1 text-zillow-ink">{BANGALORE_SPEC_SUGGESTION.ifSheLikes}</div>
                </div>
                <div className="rounded-md border border-zillow-gray-border bg-white px-3 py-2 text-xs leading-relaxed">
                  <div className="font-semibold text-zillow-slate">↳ If she doesn&apos;t</div>
                  <div className="mt-1 text-zillow-ink">{BANGALORE_SPEC_SUGGESTION.ifSheDoesnt}</div>
                </div>
              </div>
            </div>
          </div>
...
function MeetingScript({ meeting }: { meeting: OneOnOneMeeting }) {
  const showBangalorePanel =
    meeting.weekNumber === 1 &&
    meeting.type === "1:1" &&
    meeting.cards.includes("card-india-baseline");
  ...
  return (
    ...
    >
      ...
      {/* Bangalore questions panel — only shown in Week 1 1:1 */}
      {showBangalorePanel && (
        <div className="mt-6">
          <BangaloreQuestionsPanel />
        </div>
      )}
