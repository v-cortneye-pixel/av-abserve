...
export default class QREM {
    ...
    async dailyUpdate() {
        ...
        for (const system of systems) {
            ...
            // Use core status if available, fallback to system status
            const actualCoreStatus = coreStatusMap.get(system.name);        

            // Check if there are issues:
            // - Core status: code 2 = healthy, anything else = problem
            // - System status: code 0 = healthy, anything else = problem  
            const coreHasIssues = actualCoreStatus && actualCoreStatus.code !== 2;
            ...
            if (coreHasIssues || systemHasIssues) {
                ...
                if (coreHasIssues) {
                    dailyUpdateData[siteName][system.name].issues.push({
                        name: 'Core',
                        message: actualCoreStatus.message,
                        details: actualCoreStatus.details || 'Core processor issue'
                    });
                }
                
                // Also check peripheral status issues
